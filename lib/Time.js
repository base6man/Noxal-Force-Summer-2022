class Time{
    ...
    stopFunctions(parent, funcName){
        // Doesn't work
        //let tempList = JSON.parse(JSON.stringify(this.waitFunc));

        let tempList = [...this.waitFunc];
        let list1 = tempList.filter(item => (funcName !== item.funcName && funcName) || (item.parent !== parent && parent));

        //console.log(list1, tempList, this.waitFunc, parent, funcName);
        this.waitFunc = list1;
    }
    ...
    stopFunctionsWithKeyword(parent, keyword){
        ...
        //let tempList = JSON.parse(JSON.stringify(this.waitFunc));
        let tempList = [...this.waitFunc];
        let list1 = tempList.filter(item => (!keyword.test(item.funcName)) || (item.parent !== parent && parent));
        let list2 = [...list1]
        
        console.log(list1, list2, tempList, this.waitFunc, parent, keyword);
        this.waitFunc = list2;
